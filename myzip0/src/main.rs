...
fn main() -> std::io::Result<()> {
    ...
    local_header.extend(&file_size.to_le_bytes());
    local_header.extend(&file_size.to_le_bytes());
    ...
    central_dir.extend(&file_size.to_le_bytes());
    central_dir.extend(&file_size.to_le_bytes());
    ...
    end_central_dir.extend(&(local_header.len() as u32 + file_data.len() as u32).to_le_bytes());
    end_central_dir.extend(&0u16.to_le_bytes());
